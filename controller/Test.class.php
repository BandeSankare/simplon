    ...
    class Test extends Controller{
		...
		public function add(){
            ...
            if(isset($_POST['valider']))//'valider' est le name du champs submit du formulaire add.html
            {
                extract($_POST);
                $data['ok'] = 0;
                if(!empty($refP) && !empty($libProd) && !empty($qtStock) && !empty($libC)) {
                    $ok = $tdb->addTest($refP, $libProd, $qtStock, $libC);
                    $data['ok'] = $ok;
                }
		...
		public function update(){
            ...
            if(isset($_POST['modifier'])){
                extract($_POST);
                if(!empty($id) && !empty($refP) && !empty($libProd) && !empty($qtStock) && !empty($libC)) {
                    $ok = $tdb->updateTest($id, $refP, $libProd, $qtStock, $libC);
                }
            }
